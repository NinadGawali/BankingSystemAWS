class AdminComponent {
    ...
    async createAdminPage() {
        ...
        adminPage.innerHTML = `
            ...
            <div class="admin-tabs">
                <div class="tab-links">
                    ...
                    <a href="#" class="tab-link" data-tab="hashes">Transaction Hashes</a>
                </div>
                ...
                <div class="tab-content">
                    ...
                    <!-- transaction hashes tab -->
                    <div id="hashes-tab" class="tab-pane">
                        <h3>Transaction Hashes</h3>
                        <div class="action-buttons">
                            <button id="refresh-hashes" class="btn btn-primary"><i class="fas fa-rotate"></i> Refresh</button>
                        </div>
                        <div id="hashes-list">
                            <div class="empty-state">No hashes loaded</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        const refreshBtn = document.getElementById('refresh-hashes');
        if (refreshBtn) {
    ...
    async loadSystemStats() {
        ...
        async loadHashes(){
                try {
                        const resp = await fetch('/api/v1/accounts/transaction-hashes?limit=200', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}`}});
                        const list = document.getElementById('hashes-list');
                        if(!resp.ok){ list.innerHTML = '<div class="empty-state">Failed to load hashes</div>'; return; }
                        const data = await resp.json();
                        const hashes = data.hashes || [];
                        if(hashes.length === 0){ list.innerHTML = '<div class="empty-state">No hashes found</div>'; return; }
                        list.innerHTML = `
                                ...
                                <tbody>
                                    ${hashes.map(h => `
                                        <tr>
                                            ...
                                            <td>${h.from_account_number || h.from_account_id || '-'}</td>
                                            <td>${h.to_account_number || h.to_account_id || '-'}</td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                            </table>
                        `;
                } catch(e){ console.error('Error loading hashes', e); }
        }
